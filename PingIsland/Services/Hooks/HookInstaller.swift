...
struct HookInstaller {
    ...
    static func bridgeHealthStatus() -> BridgeHealthStatus {
        ...
        guard FileManager.default.fileExists(atPath: socketPath) else {
            return BridgeHealthStatus(
                isHealthy: false,
                message: AppLocalization.string("Bridge 监听尚未启动，请保持 Auralink 正在运行后重试")
            )
        }
        ...
        guard runBridgeLauncherHealthCheck(launcherURL) else {
            return BridgeHealthStatus(
                isHealthy: false,
                message: AppLocalization.string("Bridge launcher 自检失败，请重启 Auralink 后重新安装 Hooks")
            )
        }
    ...
    private static func managedMarker(for profile: ManagedHookClientProfile) -> String {
        "Auralink managed integration: \(profile.id)"
    }
    ...
    static func managedPluginSource(for profile: ManagedHookClientProfile) -> String {
        ...
        return """
        // \(marker)
        // Generated by Auralink. Reinstall from Island settings if you need to refresh it.
        ...
        export const server = async ({ client, serverUrl }) => {
          ...
          return {
          name: "Auralink",
          description: "Forward OpenCode hook events to Auralink.",
          event: async ({ event }) => {
    ...
    static func managedPluginDirectoryFiles(for profile: ManagedHookClientProfile) -> [String: String] {
        ...
        name: ping_island
        version: 1.0.0
        description: Forward Hermes Agent plugin hooks to Auralink
        provides_hooks:
          - on_session_start
          - pre_llm_call
          - pre_tool_call
    ...
    static func managedHookDirectoryFiles(
        ...
        ---
        name: \(hookName)
        description: "Forward OpenClaw internal hook events to Auralink"
        metadata:
        ...
        const handler = async (event) => {
          ...
          try {
            ...
            // OpenClaw hooks should never fail because Auralink is unavailable.
          }
        };
