...
- If you change realtime notification source presentation, keep it separate from plugin cards: update `ClientProfileRegistry`, the dedicated `RealtimeNotificationsSettingsView`, the mascot defaults/overrides, and `NotchView`'s notification-source/indicator ears together.
...
- The Plugins settings tab is a slot-assignment surface plus a read-only island-tool list. Plugin processes are available by default; only `PluginSlotArbiter` decides which two tools appear in the closed Island ears.
...
- `SettingsCategory.integration` no longer exists. Core hook notification sources live in the 实时通知 settings tab and use mascot imagery by default; user-facing island tools live in 插件; approval routing, IDE extensions, and system permissions are in 通用.
