...
- If you change realtime notification source presentation, keep it separate from plugin cards: update `ClientProfileRegistry`, `AppSettings.realtimeNotificationIconStyles`, the dedicated `RealtimeNotificationsSettingsView`, and `NotchView`'s notification-source/indicator ears together.
...
- `SettingsCategory.integration` no longer exists. Core hook notification-source icon choices live in the 实时通知 settings tab; user-facing island tools live in 插件; approval routing, IDE extensions, and system permissions are in 通用.
